...
import {
  products,
  challenges,
  retrieveBlueprintChallengeFile,
} from "../data/datacache";
...
export const emptyUserRegistration =
...
export const forgedFeedbackChallenge =
  () => (req: Request, res: Response, next: NextFunction) => {
...
export const captchaBypassChallenge =
  () => (req: Request, res: Response, next: NextFunction) => {
...
export const registerAdminChallenge =
  () => (req: Request, res: Response, next: NextFunction) => {
...
export const passwordRepeatChallenge =
  () => (req: Request, res: Response, next: NextFunction) => {
...
export const accessControlChallenges =
  () =>
  ({ url }: Request, res: Response, next: NextFunction) => {
    ...
    challengeUtils.solveIf(challenges.web3SandboxChallenge, () => {
      return utils.endsWith(url, "/11px.png");
    });
    ...
    challengeUtils.solveIf(challenges.retrieveBlueprintChallenge, () => {
      return utils.endsWith(url, retrieveBlueprintChallengeFile ?? undefined);
    });
...
export const errorHandlingChallenge =
  () =>
  (
    err: unknown,
    req: Request,
...
export const serverSideChallenges =
  () => (req: Request, res: Response, next: NextFunction) => {
  ...
  if (token) {
    ...
    if (algorithm === "HS256") {
      challengeUtils.solveIf(challenge, () => {
        return (
          hasAlgorithm(token, algorithm) &&
          hasEmail(decoded as { data: { email: string } }, email)
        );
      });
      return;
    }
...
export const databaseRelatedChallenges =
  () => (req: Request, res: Response, next: NextFunction) => {
    if (
    ...
    if (urlForProductTamperingChallenge) {
      if (
        ...
        if (
          utils.contains(
            osaft.description,
            `<a href="${config.get<string>("challenges.overwriteUrlForProductTamperingChallenge")}" target="_blank">`,
          )
        ) {
...
function typosquattingAngularChallenge() {
  ...
  ComplaintModel.findAndCountAll({
    where: { message: { [Op.like]: "%ngy-cookie%" } },
  })
    .then(({ count }: { count: number }) => {
      if (count > 0) {
        challengeUtils.solve(challenges.typosquattingAngularChallenge);
...
function leakedApiKeyChallenge() {
  FeedbackModel.findAndCountAll({
    where: {
      comment: { [Op.like]: "%6PPi37DBxP4lDwlriuaxP15HaDJpsUXY5TspVmie%" },
    },
  })
    .then(({ count }: { count: number }) => {
      if (count > 0) {
  ...
  ComplaintModel.findAndCountAll({
    where: {
      message: { [Op.like]: "%6PPi37DBxP4lDwlriuaxP15HaDJpsUXY5TspVmie%" },
    },
  })
    .then(({ count }: { count: number }) => {
      if (count > 0) {
